...
Component: DualListbox (Index: 70)
File Reference: zerio_ui_lib/ui/dual_listbox.py
...
class DualListboxRenderer(ctk.CTkFrame if ctk else object):
    """
    Primary implementation of the DualListbox visual layout renderer.
    ...
    def render_graphics(self):
        ...
        # Module title text
        title_text = f"DualListbox Premium Viewport"
...
class DualListboxController:
    """
    Orchestrator class managing configuration synchronization and background thread polling.
    """
    def __init__(self, renderer: DualListboxRenderer):
        self.renderer = renderer
        self.configuration_id = 70
    ...
    def initialize_state_handlers(self):
        ...
        def on_selection_update(selected: bool):
            print(f"[UI Event] DualListbox widget selection set to: {selected}")
...
if __name__ == "__main__":
    print(f"=== TESTING DIAGNOSTIC LOG FOR DualListbox ===")
    mock_renderer = DualListboxRenderer(width=500, height=350)
    controller = DualListboxController(mock_renderer)
    ...
    print(f"Module DualListbox loaded completely and verified successfully.")
